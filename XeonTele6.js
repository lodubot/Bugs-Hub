...
module.exports = XeonBotInc = async (XeonBotInc, bot) => {
    ...
    try {
        ...
        const username = XeonBotInc.message.from.username ? XeonBotInc.message.from.username : "Megahubbots";
...
break;
    ...
    return XeonBotInc.telegram.sendMessage(
        XeonBotInc.chat.id, 
        `🚫 *You are not authorized to use this command\\.*\n\n` +
        `📩 *Please contact the developer to buy:* @Megahubbots\n\n` +
        `💰 *Price/Harga:*\n` +
        `✅ *Access permanent:* 5$\n` +
        `✅ *Resell permanent:* 10$\n` +
        `✅ *Script no enc 100%:* 50$`,
        {
            reply_markup: {
                inline_keyboard: [
                    [{ text: "📢 Telegram Group", url: "https://t.me/Nextgenroom" }],
                    [{ text: "▶️ YouTube Channel", url: "https://youtube.com/@Freenethubtech" }],
                    [{ text: "📱 WhatsApp Channel", url: "https://whatsapp.com/channel/0029VaDnY2y0rGiPV41aSX0l" }]
                ]
            },
            parse_mode: "MarkdownV2"
        }
